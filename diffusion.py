...
#     def forward(self, pred, targ, weighter=1.0):
#         loss = self._loss(pred, targ) # 在基类中实现
#         WeightedLoss = (loss * weighter).mean()
#         return WeightedLoss
...
# # denoise network
# class MLP(nn.Module):
#     def __init__(self, state_dim, a_dim, hidden_dim, device, t_dim=16):
#         super(MLP, self).__init__()
        
#         self.a_dim = a_dim
#         self.t_dim = t_dim # temporal embedding dimension
#         self.device = device
...
#             nn.Mish(), # Transformer use Mish as active function normally
#             nn.Linear(t_dim*2, t_dim),
#         )

#         input_dim = state_dim + a_dim + t_dim
...
#         )
#         # output layer
#         self.final_layer = nn.Linear(hidden_dim, a_dim)
...
#         """
#         beta_schedule: "linear", "cosine", "cosine_with_restarts", 如何离散化时间
#         """
...
#             # beta_t = betas[i] = betas[i - 1] + (beta_max - beta_min) / (T - 1)
#             betas = torch.linspace(0.0001, 0.02, self.T, dtype=torch.float32)
        
#         alphas = 1. - betas
#         alphas_cumprod = torch.cumprod(alphas, axis=0) # [1, 2, 3] -> [1, 2, 6] cumulative product
#         alphas_cumprod_prev = torch.cat([torch.ones(1, device=kwargs["device"]), alphas_cumprod[:-1]]) # previous [1, 2, 3] -> [1, 1, 2]
...
#         # loss function
#         self.loss_fn = Losses[loss_type]() # loss_type 
...
#     def predict_strat_from_noise(self, x, t, pred_noise):
#         return (
...
#         x_recon = self.predict_strat_from_noise(x, t, pred_noise)
#         x_recon.champ_(-1, 1) # "_" -> inplace
#         model_mean, posterior_variance, posterior_log_variance = self.q_posterior(x_recon, x, t)

#         return model_mean, posterior_log_variance

#     def p_sample(self, s, x, t):
#         b, *_, device = *x.shape, x.device
#         model_mean, model_log_variance = self.p_mean_variance(x, t, s)
#         noise = torch.randn_like(x) # shape same like x

#         # when t == 0, model_log_variance = 0, so we don't need to add noise
#         nonzero_mask = (
#             (1 - (t==0).float()).reshape(b, *((1,)*(len(x.shape)-1)))
#         )
#         return model_mean + nonzero_mask * (0.5 * model_log_variance.exp() * noise)

#     def p_sample_loop(self, state, shape, *arg, **kargs):
#         device = self.device # GPU
#         batch_size = state.shape[0]
#         x = torch.randn(shape, device=device, requires_grad=False) # initial noise

#         for i in reversed(range(0, self.T)):
#             t = torch.full((batch_size,), i, device=device, dtype=torch.long)
#             x = self.p_sample(x, t, state) # revise diffusion process 
#         return x

#     def sample(self, state, *arg, **kargs):
#         batch_size = state.shape[0]
#         shape = [batch_size, self.action_dim] # noise
#         action = self.p_sample_loop(state, shape, *arg, **kargs) # revise diffusion process for action
#         return action.clamp(-1, 1) # 在RL中都会对action做一个clip

#     # ---------------------------------- train ----------------------------------- #

#     def forward(self, state, *arg, **kargs):
#         return self.sample(state, *arg, **kargs)
...
#         x_recon = self.model(x_noisy, t, state) # predict noise
        
#         loss = self.loss_fn(x_recon, noise, weights)

#         return loss

#     def loss(self, state, x, weights=1.0):
#         # x -> x_0
#         batch_size = state.shape[0]
...
#     model = Diffusion(loss_type='l2', obs_dim=11, act_dim=2, hidden_dim=256, T=1000, device=device).to(device)
#     action = model(state)
